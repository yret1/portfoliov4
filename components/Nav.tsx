...
const Nav = () => {
  ...
  if (!smallScreen) {
    return (
      <>
        <motion.section
          initial={{ top: projectview ? "2vh" : "75vh", opacity: 1 }}
          animate={{ top: projectview ? "2vh" : "75vh", opacity: 1 }}
          exit={{ top: projectview ? "2vh" : "75vh", opacity: 1 }}
        ...
        >
          <Link href="/">
            <Image
              ...
              width={50}
              height={50}
              className={`md:w-28 md:h-28 transition-opacity duration-200  ${
                scroll ? "opacity-35" : "opacity-100"
              }`}
              alt="Logo"
            />
          </Link>
        ...
        //Hantera animationer för navbar beroende på vilken sida användaren är på
          initial={{ top: projectview ? "10vh" : "91.5vh" }}
          animate={{ top: projectview ? "10vh" : "91.5vh" }}
          exit={{ top: projectview ? "10vh" : "91.5vh" }}
          transition={{ duration: 1.2, ease: "easeInOut" }}
          id="linksone"
          className={`flex flex-col fixed left-96 transition-opacity z-50 duration-200  ${
            scroll ? "opacity-35" : "opacity-100"
          }`}
        >
          <Link href={"/about"}>
            <p className="flex items-center justify-start text-[12px] hover:scale-110   gap-2">
              <span className="font-thin text-[12px] z-50 text-opacity-50">
                01
              </span>
              about
            </p>
        ...
        <header
          className={`hidden md:flex transition-all z-40 w-screen fixed bottom-0 left-0 justify-between px-10 py-4 items-end`}
        >
          ...
          <section className="flex flex-col justify-end items-center p-10">
            <Link href={"/contact"}>
              <p className="flex hover:scale-110 items-center justify-start text-[12px]  gap-2">
                <span className="font-thin text-[12px] text-opacity-50">
                  03
                </span>
                contact
              </p>
            ...
            <a href="" referrerPolicy="no-referrer" target="_blank">
              <p className="flex items-center hover:scale-110  justify-start text-[12px]  gap-2">
                <span className="font-thin text-[12px] text-opacity-50">
                  05
                </span>
                linkedin
      ...
      <header className="w-screen md:hidden fixed top-0 z-40 left-0 justify-between items-center flex">
        ...
        <section className="p-5 z-40">
          ...
          >
            <span
              id="arrow"
              className={`h-1 w-8 rounded-full bg-white transform ${
                menuOpen ? "-rotate-z-[22deg]" : ""
              } transition-all`}
            ></span>
            <span
              id="arrow"
          ...
          {menuOpen && (
            ...
            >
              <section className="flex flex-col justify-start gap-6 p-10">
                ...
                <section className="flex flex-col">
                  <Link href={"/about"}>
                    <p className="flex items-center justify-start text-lg  gap-2">
                      <span className="font-thin text-[12px] text-opacity-50">
                        01
                      </span>
                      About
              ...
              <footer className="fixed bottom-0 w-full flex justify-center items-center p-4">
                <p className="text-center text-opacity-20 text-lg font-thin">
                  © 2024 Simon Gustavsson
                </p>
              </footer>
            </motion.section>
          )}
